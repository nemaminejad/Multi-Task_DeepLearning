...
class MultiJoinedDNNClassifier(BaseEstimator, ClassifierMixin):
...
#                                  kernel_regularizer = self.weight_regularizer,name = "output" ) # no max norm for last layer
        ...
        training_op = optimizer.minimize(loss_1 + loss_2)
...
#         running_vars_initializer = tf.variables_initializer(var_list=running_vars)
        ...
        self._Y_proba1,self._Y_proba2, self._loss = Y_proba1, Y_proba2, joint_loss
        self._training_op, self._accuracy1, self._accuracy2 = training_op,accuracy1, accuracy2
        ...
        with self._session.as_default() as sess:
            ...
            for epoch in range(n_epochs):
                ...
                if X_valid is not None and y_valid1 is not None and y_valid2 is not None:
                    loss_val, acc_val1, acc_val2 = sess.run([self._loss,self._accuracy1, self._accuracy2],
                    ...
                    print("{}\tValidation loss1: {:.6f}\tBest loss: {:.6f}\tAccuracy1: {:.2f}\tAccuracy2: {:.2f}%".format(
                        epoch, loss_val, best_loss, acc_val1 * 100, acc_val2 * 100))
                    ...
                    loss_train, acc_train1, acc_train2= sess.run([self._loss,  self._accuracy1, self._accuracy2],
                                                               feed_dict = {self._X:X_batch, self._y1:y_batch1 , self._y2:y_batch2})  #, self._weight_per_label: sample_weight})
                    print("{}\tLast training batch loss: {:.6f}\tAccuracy1: {:.2f}\tAccuracy2: {:.2f}%".format(
                        epoch, loss_train, acc_train1 * 100, acc_train2 * 100))
            runtime = time.strftime('_%x_%X') 
            runtime = runtime.replace("/","-") 
            runtime = runtime.replace(":","-")
            save_model_path = os.path.join("//saved_models/saved_nn/%s"%runtime)   
